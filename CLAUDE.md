...
| `Sources/Model/GameController.swift` | Actor: game loop, input handling, state machine (`transition(to:)`, `validTransitions`), scoring, `log` method |
| `Sources/Model/GameState.swift` | Game state enum with validated transition graph — all state changes go through `transition(to:)` in GameController |
